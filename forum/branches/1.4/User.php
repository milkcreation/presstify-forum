...
class User extends \tiFy\App\Factory
{
    ...
    {
        $user_status = get_user_option( 'tify_forum_status', $user_id );
        $statuses = self::getStatuses();
    ...
    {        
        if( ! $activation_key = get_user_option( 'tify_forum_key', $user_id ) ) :
    ...
    {                
        if( ! $unsubscribe_token = get_user_option( 'tify_forum_unsub', $user_id ) ) :
            $unsubscribe_token = wp_generate_password( 48, false, false );
            update_user_option( $user_id, 'tify_forum_unsub', $unsubscribe_token );
        endif;
